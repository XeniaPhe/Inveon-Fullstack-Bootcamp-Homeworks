...
internal class LessThanOrEqualStrategy<TEntity, TProperty>() : IFilterOperatorStrategy<TEntity, TProperty>
{
    public Expression<Func<TEntity, bool>> GetFilterExpression(TProperty filter, Expression<Func<TEntity, TProperty>> filteredPropertyExpr)
    {
        ...
        return IFilterOperatorStrategy<TEntity, TProperty>.CreateFilterExpression(filter, filteredPropertyExpr, Expression.LessThanOrEqual);
    }
}